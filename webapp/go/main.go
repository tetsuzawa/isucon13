...
GROUP BY emoji_name`
	...
	if err := dbConn.SelectContext(ctx, &tcs, "SELECT user_id, COUNT(*) AS total_comment_count, SUM(tip) AS total_tip FROM livecomments c ON INNER JOIN livestreams l c.livestream_id = l.id GROUP BY l.user_id"); err != nil {
